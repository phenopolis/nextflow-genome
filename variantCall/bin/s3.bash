...
nxf_s3_upload() {
    ...
    for name in $(eval "ls -1d $pattern");do
      if [[ -d "$name" ]]; then
        /home/ec2-user/miniconda/bin/aws $aws_profile s3 cp --only-show-errors --recursive --storage-class STANDARD "$name" "$s3path/$name"
      else
        echo "uploading $name to $2"
        cmd="/home/ec2-user/miniconda/bin/aws $aws_profile s3 cp --only-show-errors --storage-class STANDARD $name $s3path/$name"
        #/home/ec2-user/miniconda/bin/aws $aws_profile s3 cp --only-show-errors --storage-class STANDARD $name $s3path/$name
        eval "$cmd"
        echo 'done'
      fi
    done
