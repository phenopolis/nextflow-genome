...
NXF_VER=20.01.0 nextflow run variantCall.nf -bucket-dir s3://phenopolis-nextflow/nextflow-work-test -with-report report.html
*/
// determines build
...
Channel.fromFilePairs(params.input_bam).into {Bam_ch1; Bam_ch2}
Human_ref_withdict_ch = Channel.value(file("${gatk_bundle}/${human_ref_base}*"))
Calling_interval_list_ch = Channel.value(file("${gatk_bundle}/${calling_interval_list}"))
...
process 'CNN_1_HaplotypeCallerGvcf' {
  ...
  input:
    tuple val(sampleId), path(input_bam) from Bam_ch1
...
process 'CNN_3_filterCNNVariants' {
  ...
  input:
    ...
    file '*' from GATK_ch
  output:
    path("${sampleId}.CNN.filtered.vcf.gz*") into CNN_filtered_ch
  ...
  resource_list=\$(printf " -resource %s" "\${resource_list[@]}")
...
process '2A_HaplotypeCallerGvcf' {
  ...
  container params.gatk_docker
  input:
    tuple val(sampleId), path(input_bam) from Bam_ch2
    file '*' from Human_ref_withdict_ch
    file '*' from Calling_interval_list_ch
  output:
